 ...
 * Version:     1.03
 * Description: The Reversi game, with computer moves
 */
...
int main(void) {
    ...
    while(!gameOver) {
        ...
        if(turn == computerColor && gameOver == false) { 
            ...
            //printf("\nmax score for row %d col %d : %d \n", computerRow, computerCol, maxScore);
            ...
            //checkValidAndFlip(board, dimension, computerRow, computerCol, computerColor, true);
        ...
        if(!gameOver) {
            ...
            if(findMoves(board, dimension, findOpposite(turn))) {
            ...
            printf("\n%c player wins.", winner);
            printf("\n");

        }
        
        // set ebverything to false again;
...
bool moveValid(char board[][26], int dimension, char colour, int rows, int cols) {
    ...
    if (/*board[rows][cols] != 'W' && board[rows][cols] != 'B'*/board[rows][cols] == 'U') {
...
bool checkLegalInDirection(char board[][26], int dimension, int row, int col, char colour, int deltaRow, int deltaCol) {
    ...
    if (positionInBounds(dimension, newRow, newCol)) {
        ...
        // check for color
        if (/*board[newRow][newCol] != colour && board[newRow][newCol] != 'U'*/ board[newRow][newCol] == findOpposite(colour)) {
...
int flipInDirection(char board[][26], int dimension, int row, int col, char colour, int deltaRow, int deltaCol, bool flip) {  
    ...
    do {
        ...
        if(flip) {
            ...
            if(/*board[newRow][newCol] != 'U'*/board[newRow][newCol] == findOpposite(colour)) {
...
/*
            for(int rows = 0; rows < dimension; rows++) {
                for(int cols = 0; cols < dimension; cols++) {
                    ...
                    if(moveValid(board, dimension, findOpposite(turn), rows, cols)) {
                        
                        for(int i = -1; i < 2; i++) {
                            for(int j = 1; j > -2; j--) {
                                if(checkLegalInDirection(board, dimension, rows, cols, findOpposite(turn), j, i)) {
                                    int s = flipInDirection(board, dimension, rows, cols, computerColor, j, i, false);
                    ...
                    // turn stays same: computer has no moves
                    else if(moveValid(board, dimension, turn, rows, cols)) {
                        
                        for(int i = -1; i < 2; i++) {
                            for(int j = 1; j > -2; j--) {
                                if(checkLegalInDirection(board, dimension, rows, cols, turn, j, i)) {
                                    int p = flipInDirection(board, dimension, rows, cols, turn, j, i, false);
            ...
            else if(foundAvailableMove == true && foundAvailableMoveOpposite == false) {
                turn = turn;
                printf("%c player has no valid move.", findOpposite(turn));
            }
            
            // neither player has a turn --> GAME OVER
            if(foundAvailableMove == false && foundAvailableMoveOpposite == false) {
                gameOver = true;
            }
